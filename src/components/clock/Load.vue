<template>
  <div class="login_container all">
    <div class="login_box">
      ...
      <a-form>
        ...
        <a-form-item class="btns">
          <a-button type="primary" class="btns_marign" @click="login" :loading="loginLoading"
            >挂载</a-button
          >
          <a-button type="primary" class="btns_marign" @click="resetLoginForm()" :loading="resetLoading">重置</a-button>
    ...
    >
      ...
      <p>第一次挂载时会有试运行结果发送到设置的邮箱！成功即表明加入任务，失败请检查密码后重试！</p>
      <p>退出挂载，输入正确的账户信息后，使用重置按钮！</p>
      <p>信息化门户地址(网上办事大厅):<a target="_blank" href="http://auth.zjut.edu.cn/zfca/retakeusername.do">跳转</a></p>
...
export default {
  ...
  methods: {
    ...
    async resetLoginForm() {
      ...
      const result = await this.$http.post("http://webproxy2.jxhonline.com:20270/aff/clock/out", 
      this.loginForm
      );
    ...
    async login() {
      ...
      const result = await this.$http.post("http://webproxy2.jxhonline.com:20270/aff/clock/register", 
      this.loginForm
      );
      if(result.data === 'success') {
